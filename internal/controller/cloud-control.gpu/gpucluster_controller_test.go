...
var _ = Describe("GpuCluster Controller", func() {
	Context("When reconciling a resource", func() {
		...
		gpucluster := &cloudcontrolgpuv1beta1.GpuCluster{}

		BeforeEach(func() {
			By("creating the custom resource for the Kind GpuCluster")
			err := k8sClient.Get(ctx, typeNamespacedName, gpucluster)
			if err != nil && errors.IsNotFound(err) {
				resource := &cloudcontrolgpuv1beta1.GpuCluster{
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance GpuCluster")
			Expect(k8sClient.Delete(ctx, resource)).To(Succeed())
		})
		It("should successfully reconcile the resource", func() {
			By("Reconciling the created resource")
			controllerReconciler := &GpuClusterReconciler{
				Client: k8sClient,
